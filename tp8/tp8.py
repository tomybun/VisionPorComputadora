...
def dibuja(event, x, y, flags, param):
	global xy_iniciales, drawing, img, foto_recortada, img_copia, k, puntos_seleccionados, contador_puntos, puntos_destino, modo_afin
	...
	if event == cv2.EVENT_LBUTTONDOWN:
		...
		#Si esta en modo homografia	
		if modo_homografia is True:
	...
	elif event == cv2.EVENT_MOUSEMOVE and modo_afin is False and modo_homografia is False:
		if drawing is True:
...
#Bandera que indica si se está en modo homografia, ya que cambia el dibujo
modo_homografia= False

#Bandera que controla que se entre una sola vez al modo homografia
img_nueva = False
...
while True:
	...
	#Transformacion afin
		...
		if contador_puntos == 3:
			...
			imagen_resultante = insertar_imagen(img_aux, imagen_insertar, puntos_destino)
			
			#Para pegar la imagen insertada en la imagen de abajo
			overlay = imagen_resultante.copy()  
			img = cv2.addWeighted(overlay, 1, img_aux, 0, 0)	
						
	
	#Homografia
	elif k == ord('h') or modo_homografia:
			
		#Se setea el modo homografia
		modo_homografia = True
		
		if img_nueva is False:
			#Lectura de la imagen que se va a modificar
		...
		if contador_puntos == 4:
			...
			#puntos_destino = np.array([[0, 0], [ancho - 1, 0], [ancho - 1, alto - 1], [0, alto - 1]], dtype=np.float32)				
			...
			cv2.imwrite('o_rectificacion.png', img_rectificada)
			
			img     = img_rectificada.copy()	
